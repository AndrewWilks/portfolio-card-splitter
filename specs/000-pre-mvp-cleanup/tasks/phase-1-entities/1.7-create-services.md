...
- [ ] Create `CardAccountService` with repository DI
- [ ] Implement `createCardAccount(request, settingsOverrides?)` - Creates account + settings
- [ ] Implement `listCardAccounts(ownerId)` - List user's card accounts
- [ ] Implement `getCardAccount(id, ownerId)` - Get single account with ownership check
- [ ] Implement `updateCardAccount(id, request, ownerId)` - Update with ownership check
- [ ] Implement `deleteCardAccount(id, ownerId)` - Soft delete with transaction check
- [ ] Create `CardAccountSettingsService` with repository DI
- [ ] Implement `getSettings(cardAccountId)` - Get settings for account
- [ ] Implement `updateSettings(cardAccountId, request)` - Update settings
- [ ] Implement `resetToDefaults(cardAccountId, preset?)` - Reset with optional preset
- [ ] Create `CardService` with repository DI
- [ ] Implement `createCard(request)` - Create card with validation
- [ ] Implement `listCards(cardAccountId)` - List cards for account
- [ ] Implement `listCardsByMember(memberId)` - List cards by member
- [ ] Implement `getCard(id)` - Get single card
- [ ] Implement `updateCard(id, request)` - Update card
- [ ] Implement `deleteCard(id)` - Soft delete card
- [ ] Export services from `backend/services/index.ts`
- [ ] Register services in `backend/di/services.ts`
- [ ] Create test files for all three services
