...
import { CardAccountService, CardService } from "../../services/index.ts";
import {
...
Deno.test(
  ...
  async () => {
    await withCleanDatabase(async () => {
      ...
      const { cardAccount: account1 } =
        await cardAccountService.createCardAccount(
          {
            name: "Account 1",
            issuer: "Bank 1",
            last4: "1111",
